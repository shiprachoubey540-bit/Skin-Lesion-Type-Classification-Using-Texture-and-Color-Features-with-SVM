...
# --- FIX 1: Guard xgboost import so a missing install gives a clear message ---
try:
    import xgboost  # noqa: F401  (just checking it's available)
...
st.markdown("""
    <style>
    [data-testid="stApp"] { background-color: #0f172a; color: #f8fafc; }
...
# --- FIX 2: Warn early if xgboost is not installed ---
if not XGBOOST_AVAILABLE:
    st.warning(
        "⚠️ **XGBoost is not installed** in this environment. "
        "Run `pip install xgboost` (or add it to `requirements.txt`) and restart the app. "
        "Only the SVM model will be available until then.",
        icon="📦"
    )

# --- LOADING DIRECTLY FROM THE 'models/' FOLDER ---
@st.cache_resource
def load_assets():
    loaded_models = {}
    scaler = None

    # Always try to load SVM (no extra dependency)
    try:
        loaded_models["SVM"] = joblib.load('models/svm.pkl')
    except FileNotFoundError:
        st.error("❌ `models/svm.pkl` not found. Make sure the models folder is present.")

    # Only load XGBoost if the package is actually importable
    # FIX: loading xgboost.pkl without the xgboost package installed causes
    # the UnpicklingError / ModuleNotFoundError seen in the traceback.
    if XGBOOST_AVAILABLE:
        try:
            loaded_models["XGBoost"] = joblib.load('models/xgboost.pkl')
        except FileNotFoundError:
            st.error("❌ `models/xgboost.pkl` not found.")
    else:
        st.info("ℹ️ XGBoost model skipped (package not installed).")

    try:
        scaler = joblib.load('models/scaler.pkl')
    except FileNotFoundError:
        st.error("❌ `models/scaler.pkl` not found.")

    return loaded_models if loaded_models else None, scaler

models, scaler = load_assets()

# --- FIX 3: Define selected_name at top level so tab1 can always access it ---
selected_name = "SVM"  # safe default
...
if models:
    with st.sidebar:
        ...
        # Only show models that were successfully loaded
        available_models = list(models.keys())
        selected_name = st.selectbox("⚙️ Select Model", available_models)
...
def extract_features(img_input):
    ...
    for prop in ["contrast", "dissimilarity", "homogeneity", "energy", "correlation"]:
        ...
        glcm_summary[prop] = float(values.mean())
    ...
    all_features = glcm_feats + hsv_feats + [asymmetry, border]
...
with tab1:
    ...
    with col_left:
        ...
        if uploaded_file:
            ...
            # FIX 4: Guard against missing models/scaler before classifying
            if models and scaler and selected_name in models:
                if st.button("🚀 Classify Image", type="primary"):
                    with st.spinner('Analyzing...'):
                        ...
                        with col_right:
                            st.markdown(f"""
                            <div style="background-color: #1a1f2e; padding: 25px; border-radius: 12px;
                                        text-align: center; border: 1px solid #00d1b2;">
                            ...
                            st.markdown(
                                f"""<div class="risk-box">
                                    ...
                                    <span style="font-size:0.85rem; color: white;">— {risk['advice']}</span>
                                </div>""",
                                unsafe_allow_html=True
                            )

                            radar_metrics = list(g_summary.keys())
                            radar_values = list(g_summary.values())
                ...
                st.error("Models or scaler could not be loaded. Please check the `models/` folder and install all dependencies.")

with tab2:
    st.subheader("Model Comparison Overview")
    ov1, ov2, ov3, ov4 = st.columns(4)
    ov1.markdown('<div class="glass-card"><p class="stat-label">Models</p><p class="stat-val">2</p></div>', unsafe_allow_html=True)
    ...
    st.plotly_chart(
        px.bar(METRICS_DATA, x="name", y="accuracy", title="Accuracy (%)",
               color="name", template="plotly_dark"),
        use_container_width=True
    )
    st.plotly_chart(
        px.bar(CLASS_DISTRIBUTION, x="count", y="name", orientation='h',
...
with tab3:
    ...
    with m_col2:
        st.info("**Classification**\n\nWe utilize a multi-model approach (SVM, XGBoost) to provide comparative diagnostic insights.")
    with m_col3:
        st.info("**Evaluation**\n\nModels are validated using 5-fold cross-validation, focusing on Specificity to reduce false alarms.")

    st.markdown(
        '<div class="warning-card">'
        '<h3 style="color: #ffa500; margin: 0;">⚠️ Medical Disclaimer</h3>'
        '<p style="color: #cbd5e0; font-size: 0.95rem; margin-top: 10px;">'
        'This application is an AI-based screening tool and <b>not a substitute for professional medical advice, '
        'diagnosis, or treatment</b>. Always seek the advice of your dermatologist with any questions regarding a skin condition.'
        '</p></div>',
        unsafe_allow_html=True
    )
